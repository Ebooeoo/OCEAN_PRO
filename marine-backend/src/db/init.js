...
async function initDB() {
  ...
  const conn = await mysql.createConnection({
    ...
    password: process.env.DB_PASSWORD || 'jypjyp123',
    charset: 'utf8mb4'
  })
  ...
  // 创建数据库
  await conn.query(`CREATE DATABASE IF NOT EXISTS marine_biodiversity CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`)
  await conn.query(`USE marine_biodiversity`)
  console.log('✅ 数据库 marine_biodiversity 已就绪')
  ...
  `)
  console.log('✅ 表 observations 已创建')
  ...
  `)
  console.log('✅ 表 activity_logs 已创建')
  ...
  // 检查生态系统是否已有数据
  const [ecoRows] = await conn.query('SELECT COUNT(*) as cnt FROM ecosystems')
  if (ecoRows[0].cnt === 0) {
    await conn.query(`
  ...
  // 检查物种是否已有数据
  const [spRows] = await conn.query('SELECT COUNT(*) as cnt FROM species')
  if (spRows[0].cnt === 0) {
    await conn.query(`
  ...
  // 检查观测记录
  const [obsRows] = await conn.query('SELECT COUNT(*) as cnt FROM observations')
  if (obsRows[0].cnt === 0) {
    await conn.query(`
      INSERT INTO observations (title, observed_at, longitude, latitude, ecosystem_id, ecosystem_name, observers, water_temp, salinity, depth, weather_condition, notes, created_by, created_at) VALUES
  ...
  // 插入活动日志
  const [logRows] = await conn.query('SELECT COUNT(*) as cnt FROM activity_logs')
  if (logRows[0].cnt === 0) {
    await conn.query(`
